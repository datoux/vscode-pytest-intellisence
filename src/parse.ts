export type Fixture = {
  ...
  export const parseCommand = (value: string | Array<string>, pythonPath: string | Array<string>) => {
    ...
    if (typeof pythonPath === "string") {
        ...
        command.pythonPath = pythonPath.join(":");
    }
  ...
  export const shouldSuggest = (lineText: string, cursorPosition: number, fullText: string): boolean => {
    ...
    let fixtures = fullText.match(/def test_[^(]+\([^})]+[\n\r\s]?\)/g)?.join(" ");
  
    if (fixtures?.includes(lineText)){    
        return true;
    }
    return false;
  };
